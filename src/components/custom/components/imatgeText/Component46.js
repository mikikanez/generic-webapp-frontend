				...
				<Box py={2}>
					<Grid container spacing={8}>
						<Grid item md={6} xs={12} display={"flex"} flexDirection={"column"} justifyContent={"space-between"}>
							<Box>
                                {valor(1, component) && (
									<Typography variant="body1" mb={4} textAlign={"left"} fontWeight={"bold"} textTransform={"unset"} color={component.dark ? theme.palette.details.main : theme.palette.secondary.main}>
                                        {valor(1, component)}
                                    </Typography>
                                )}
								{valor(0, component) && (
                                    <Typography variant="h2" mb={4} color={component.dark ? "white" : 'black'} textTransform={'unset'} fontWeight={'bold'}>{valor(0, component)}</Typography>
                                )}
                                {valor(2, component) && (
                                    <Box my={5}>
                                        <Typography variant="body2" color={component.dark ? "white" : theme.palette.background.dark}>
                                            {valor(2, component)}
                                        </Typography>
                                    </Box>
                                )}
