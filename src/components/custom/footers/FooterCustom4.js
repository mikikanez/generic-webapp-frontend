				...
				>
					<Grid item md={5} xs={12} textAlign={"left"} marginTop={5}>
						<Box textAlign={matches ? "left" : "center"}>
							<Typography variant="h3" fontWeight={"bold"} textTransform={"capitalize"} color={isDark(footerColor) ? "white" : "black"}>
								{opcions?.titol}
							</Typography>
						...
						<Stack direction={"row"} spacing={2} justifyContent={matches ? "left" : "center"} marginTop={2}>
							{opcions?.instagram && (
								<a
									href={opcions?.instagram}
									target={"_blank"}
									rel="noreferrer"
									style={{
										background: opcions?.primary,
										height: "30px",
										width: "30px",
										display: "flex",
